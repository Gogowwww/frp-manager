...
def api_config_save(iid):
    if DEMO_MODE:
        return jsonify({"ok": True, "msg": "⚠️ Mode démo — sauvegarde désactivée"})
    detect_frp(force=False)
    if iid not in INSTANCES:
        return jsonify({"ok": False, "msg": "Instance inconnue"}), 404
    inst = INSTANCES[iid]
    ...
    if inst.get("source") == "docker":
        ...
        # 1. Chercher via docker inspect → Mounts
        ...
        if status != 200 or not isinstance(inspect, dict):
            ...
            status, inspect = _docker_api("GET", f"/containers/{alt}/json")
        ...
        # 2. Fallback : /etc/frp/frpc.toml (monté dans docker-compose)
        if not cfg_path:
    ...
    if not inst.get("config"):
        return jsonify({"ok": False, "msg": "Aucun fichier de config associé à cette instance"}), 400
    cfg = Path(inst["config"])
    try:
